 ...
 * under the License.
 */
package com.aliyun.dataworks.dataservice.common.utils;
...
public class SignUtil {
    ...
    //static List<String> signHeaders = new ArrayList<>();
    //static {
    //    signHeaders.add(CommonConstants.X_CA_KEY);
    //    signHeaders.add(CommonConstants.X_CA_TIMESTAMP);
    //    signHeaders.add(CommonConstants.BASEID);
    //    signHeaders.add(CommonConstants.APPCODE);
    //}
    //
    ...
    public static String sign(String secret, String method, String path, 
        ...
        try {
            ...
            String str = buildStringToSign(method, path, headers, querys, bodys, signHeaderPrefixList);
            return new String(Base64.encodeBase64(
                    hmacSha256.doFinal(str.getBytes(Constants.ENCODING))),
                    Constants.ENCODING);
        } catch (Exception e) {
    ...
    private static String buildResource(String path, Map<String, Object> querys, Map<String, Object> bodys) {
        ...
        if (null != bodys) {
        	for (Map.Entry<String, Object> body : bodys.entrySet()) {
        ...
        for (Map.Entry<String, Object> item : sortMap.entrySet()) {
    		if (!StringUtils.isBlank(item.getKey())) {
    			if (0 < sbParam.length()) {
    				sbParam.append(Constants.SPE3);
    			}
    			sbParam.append(item.getKey());
    			if (ObjectUtils.notEqual(item.getValue(),null)) {
